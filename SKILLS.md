...
data wrangling, knowledge-graph embeddings (TransE/RotatE, e.g. via PyKEEN).

**Owns:** `ml_engine/app/utils/neo4j_client.py`,
`scripts/filter_hetionet_fragment.py`, `scripts/load_fragment.py`,
`data/processed/`, (Sprint 2+) `ml_engine/app/services/kg_embeddings.py`.
...
**Core skills:** PyTorch Geometric (GCN/GAT), Fastify, Node.js, Prisma,
API security.

**Owns:** `backend/src/server.js`, `backend/src/rules/contraindications.js`,
`backend/prisma/schema.prisma`, `ml_engine/main.py`, (Sprint 3+)
`ml_engine/app/models/ddi_gnn.py`, `ml_engine/app/services/combination_search.py`
...
**Known gotchas:**
- Never call PyTorch/PyG directly from Node — always via HTTP to
  `ml_engine`. This is a hard architectural boundary, not a preference.
...
**Core skills:** greedy set-cover / integer programming, GNNExplainer,
Streamlit or React, clinical-trial validation (reading trial data, not
running trials).

**Owns:** `ml_engine/app/services/combination_search.py` (shared with
Member B), (Sprint 4) `ml_engine/app/services/explainability.py`,
`frontend/`, (Sprint 3) `docs/validation-report.md`.

**"Done" looks like:** every returned drug combination is traceable to a
specific explanation (GNNExplainer output or a rule-based block reason);
the UI visually distinguishes "hard rule blocked this" from "model scored
this as risky."

**Known gotchas:**
- The GNN's score is an input to set-cover, never the final answer by
  itself — see CONTEXT.md's design principles section before changing
  this logic.
...
| GNN, GCN/GAT, PyTorch Geometric, DDI model training      | Members A+B      | PyTorch, PyG                     | `ml_engine/app/models/`                                     | Training/model code belongs in `ml_engine`, never `backend` |
...
| set-cover, combinatorial search, optimization             | Member C (w/ B)  | Python                           | `ml_engine/app/services/combination_search.py`               | Operates on GNN *scores*, never on raw graph structure directly |
| GNNExplainer, explainability, "why" trace                 | Member C         | PyG explainability tools         | `ml_engine/app/services/explainability.py`                   | Every prediction needs a trace — this is a hard requirement, not polish |
